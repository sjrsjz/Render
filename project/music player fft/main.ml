...
Shader_Main{
    ...
    R:scale;
    R:dt;
    vec4:stream;vec4:streamin;
    var_update(N:name)->N:={
        Frame=0;front=1
    }
    frame_start()->N:={
    
        getFFT(&"sound0",fft,-4096);
        Offset=GetFloat(&"offset");
        scale=GetFloat(&"scale");
        dt=GetFloat(&"dt");
        getVec4(&"Position2D",Position2D);
        getVec4(&"sound1",stream);
        getVec4(&"sound0",streamin);
        if(lOffset!=Offset){
            Frame=0
        };
        lOffset=Offset;
    ...
    frame_update()->N:={
        ...
        setF(&"iTime",time);
        setI(&"iFrame",Frame);
        compute(4096/256,1,1);
        playFFT(sound_buffer,stream.w,4096,1,streamin.x);
